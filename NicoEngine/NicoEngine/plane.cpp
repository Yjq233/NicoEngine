...
int CPlane::ClassifyPolygon(CPolygon &pol)
{
	...
	if (planePolys == 3) return NE_ON_PLANE;
	else if (frontPolys == 3) return NE_FRONT;
	else if (backPolys == 3) return NE_BACK;
...
int CPlane::ClassifyPoint(CVector3 &v)
{
	...
	if (distance > 0.001) return NE_FRONT;
	if (distance < -0.001) return NE_BACK;
